  ...
  return (
    <div ref={rootRef} className="pointer-events-none absolute inset-0 z-0 overflow-hidden">
      <div
        ...
        className="absolute -left-24 top-4 h-72 w-72 rounded-full bg-accent/25"
        style={{ filter: `blur(${profile.blurPx}px)` }}
      />
      <div
        data-parallax-depth={profile.reduced ? "0.1" : "0.35"}
        className="absolute right-[-6rem] top-24 h-80 w-80 rounded-full bg-primary-foreground/20"
        style={{ filter: `blur(${profile.blurPx + 8}px)` }}
      />
      <div
        data-parallax-depth={profile.reduced ? "0.18" : "0.65"}
        className="absolute bottom-[-5rem] left-1/3 h-64 w-64 rounded-full bg-accent/20"
        style={{ filter: `blur(${profile.blurPx + 6}px)` }}
      />
      ...
      {particles.map((particle) => (
        <span
          ...
          className="absolute rounded-full bg-accent/30 will-change-transform"
          style={{
      ...
      {!profile.isSmallMobile ? (
        <>
          <ShieldCheck data-parallax-depth="0.4" className="absolute left-[12%] top-[22%] size-8 text-accent/70" />
          <Landmark data-parallax-depth="0.55" className="absolute right-[14%] top-[32%] size-9 text-primary-foreground/65" />
          <BarChart3 data-parallax-depth="0.45" className="absolute left-[28%] bottom-[26%] size-8 text-accent/75" />
        </>
      ) : null}
    </div>
  )
